...
class Game {
  constructor() {
    ...
    this._filed = new Field(this._context, 0, 0, canvas.clientWidth, canvas.clientHeight, BACKGROUND_COLOR);
    this._enemy = new Player(this._context, canvas.clientWidth / 2 - PLAYER_HEIGHT, 0, PLAYER_WIDTH, PLAYER_HEIGHT, ENEMY_COLOR);
    this._player = new Player(this._context, canvas.clientWidth / 2 - PLAYER_HEIGHT, canvas.clientHeight - PLAYER_HEIGHT, PLAYER_WIDTH, PLAYER_HEIGHT, PLAYER_COLOR);
    this._ball = new Circle(this._context, BALL_COLOR, canvas.clientWidth / 2, canvas.clientHeight / 2, BALL_SIZE, BALL_SIZE, BALL_RADIUS);
  ...
  _moveEnemy() {
    if ((this._ball._x + this._enemy._width < this._canvas.width) && (this._enemy._x > 0)) {
  ...
  _update() {
    ...
    if ((this._ball._x - this._ball._width / 2 < 0) || (this._ball._x + this._ball._width / 2 > this._canvas.width)) {
      this._ball.vX = -this._ball.vX;
    }

    // Соприкосновение с ракетками
    //TODO :: проверить правильно ли обрабатываются коллизии, не могу найти ошибку
  ...
  _screenSaver() {
    if (!this._start) {
      ...
      this._context.fillRect(0, 0, canvas.clientWidth, canvas.clientHeight);
